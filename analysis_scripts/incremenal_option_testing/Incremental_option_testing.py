...
tokenizer = AutoTokenizer.from_pretrained("google/gemma-2-9b-it") #exmaple containing gemma
device = "cuda:1"
...
def final_fun_cal(df,target_region,options):
  ...
  for i,j in enumerate(sentences1):
    ...
    for input in generate_input(j,options):
      ...
      second_last_newline_index = text.rfind('\n', 0, last_newline_index)
      generated_text = text[second_last_newline_index+1 : last_newline_index].strip()
...
regions = ["Latin america", "Middle-east-Africa", "East-Asia", "North America Europe", "INDIA"]
