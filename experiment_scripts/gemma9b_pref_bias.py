...
def main(args):
    ...
    output_folder = f'response_{response_number}/pref_bias/gemma9b'
        ...
        for json_file in json_files:
            with open(os.path.join(input_folder, json_file), 'r', encoding='utf-8') as f:
                ...
                if data[0]['region'] != region:
                    continue
                for filler_prompt,typ,cut_len in zip(chat_templates_list,['one'],[191]):
                    sentences = []
                    for entry in data:
                ...
                output_csv = os.path.join(output_folder, f'gemma9b_pref_bias_{region.lower()}.csv')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script for generating responses from gemma model for the pref bias task.')
