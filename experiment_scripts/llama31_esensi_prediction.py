...
def main(args):
    ...
    def generate_prompt(prompt: str):
        ...
        """
        template= "<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\nAnswer in one word if the given cultural etiquette is (1)acceptable or (2)unacceptable.<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n'{}'<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
    ...
    # Access the grouped positive as well as negative data.
    input_folder = 'final_data_grouped/final_merged_data/GROUPED'
    output_folder = f'response_{response_number}/esensi/llama31'
    ...
    for region in regions:
        ...
        if f'llama31_{region.lower()}.csv' in existing_files:
            ...
            for json_file in json_files:
                with open(os.path.join(folder_path, json_file), 'r', encoding='utf-8') as f:
                    ...
                    for i in range(len(dataset)):
                        ...
                        region_data.append([sentences[i], data[i]['group'], data[i]['label'], response_sentences[i][114+len(sentences[i]):]])

        output_csv = os.path.join(output_folder, f'llama31_{region.lower()}.csv')
        with open(output_csv, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            # Write header
            writer.writerow(['sentence', 'group', 'true_label', 'llama31_sentence'])
            # Write the data
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script for generating responses from llama model for the esensitivity task.')
